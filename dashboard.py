# app.py
# =============================================================================
# DQ Sales Monitor — Visual Misto (Zonas + Intervalos Numéricos)
# =============================================================================
...
# --- Mantido para TEMPERATURA (Baixo/Médio/Alto) ---
...
def grouped_zone_chart(valid_zone: pd.DataFrame, quar_zone: pd.DataFrame, title: str):
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y("AvgSales:Q", title="Vendas Média"),
...
# --- NOVA FUNÇÃO: Gráfico de Barras por Intervalos Numéricos (para Fuel, Unemp, CPI) ---
def numeric_bin_chart(valid_df: pd.DataFrame, quar_df: pd.DataFrame, xcol: str, step: float, title: str):
    ...
    if len(df) == 0:
        return alt.Chart(pd.DataFrame({"msg": ["Sem dados"]})).mark_text(size=14).encode(text="msg:N").properties(height=350, title=title)
    
    # Criar bins (arredondar para o passo mais próximo)
    # Ex: se step=0.5, valor 2.7 fica 2.5. 3.1 fica 3.0
    df["Bin"] = (np.floor(df[xcol] / step) * step).round(2)
    
    # Calcular média por bin e status
...
with tab_insights:
    ...
    st.caption("Relação entre indicadores e Vendas (Temperatura em zonas; outros em intervalos numéricos).")

    if len(valid_f) == 0 and len(quar_f) == 0:
        st.warning("Sem dados para estes filtros.")
    else:
        # 1. TEMPERATURA (Mantém-se Baixa/Média/Alta)
        ...
        # 2. OUTRAS VARIÁVEIS (Intervalos numéricos simples, barras Verde vs Vermelho)
        c1, c2 = st.columns(2)
        with c1:
            # Combustível agrupado de 0.5 em 0.5
            st.altair_chart(numeric_bin_chart(valid_f, quar_f, "Fuel_Price", 0.5, "Fuel Price (Intervalos 0.5)"), use_container_width=True)
        with c2:
            # Desemprego agrupado de 1.0 em 1.0
            st.altair_chart(numeric_bin_chart(valid_f, quar_f, "Unemployment", 1.0, "Unemployment (Intervalos 1.0)"), use_container_width=True)
        
        # CPI agrupado de 10 em 10
        st.altair_chart(numeric_bin_chart(valid_f, quar_f, "CPI", 10.0, "CPI (Intervalos 10.0)"), use_container_width=True)
